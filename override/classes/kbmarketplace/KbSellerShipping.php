...
class KbSellerShipping extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'id_reference' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt',
                'required' => true),
            'is_default_shipping' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt',
                'default' => 0),
    ...
    $id_seller, $id_lang, $only_count = false, $start = null, $limit = null, $orderby
    = null, $orderway = null, $custom_filter = ''
    )
    {
        $context = new Context();
    ...
    $id_lang, $id_seller = 0, $only_system_shipping = false, $active = false, $delete
    = false, $id_zone = false, $modules_filters = Carrier::PS_CARRIERS_ONLY, $for_admin
    = false
    )
    {
        $sql = '
		SELECT c.*, cl.delay, s.id_seller 
		FROM `'._DB_PREFIX_.'carrier` c '.(($only_system_shipping === true) ? 'LEFT JOIN '
                            : 'INNER JOIN ')
