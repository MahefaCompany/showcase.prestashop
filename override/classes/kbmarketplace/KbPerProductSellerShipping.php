...
class KbPerProductSellerShipping extends ObjectModel
{
    ...
    public static function getCartProductsWithCarriers()
    {
        ...
        foreach (Context::getContext()->cart->getProducts() as $key => $product) {
            if (isset($product['id_address_delivery'])) {
                $avilable_carriers_per_product[$product['id_product']]['product']
                        = array('cart_quantity' => $product['cart_quantity'], 'id_address_delivery' => $product['id_address_delivery']);
            } else {
                $avilable_carriers_per_product[$product['id_product']]['product']
                        = array('cart_quantity' => $product['cart_quantity']);
            }
    ...
    public static function getCarriersByCondition()
    {
        ...
        if (isset(Context::getContext()->cart->id_address_delivery) // Be carefull, id_address_delivery is not usefull one 1.5
                && Context::getContext()->cart->id_address_delivery && Customer::customerHasAddress(Context::getContext()->cart->id_customer, Context::getContext()->cart->id_address_delivery
                )) {
            $id_zone = Address::getZoneById((int) Context::getContext()->cart->id_address_delivery);
        } else {
        ...
        foreach ($available_carriers as $pid => $carriers) {
            ...
            foreach ($carriers['shipping'] as $key => $carrier) {
                if (isset(Context::getContext()->cookie->kb_selected_carrier)) {
                    ...
                    if (isset($selected_carrier[$pid]) && $carrier['id_carrier']
                            == $selected_carrier[$pid]) {
                        $available_carriers[$pid]['shipping'][$key]['selected_carrier']
                                = 1;
                    }
                } else {
                    if (isset($carrier['is_default_carrier']) && $carrier['is_default_carrier']
                            == 1) {
                ...
                if ($carrier['is_free'] == 1) {
                    ...
                    if (($shipping_method == Carrier::SHIPPING_METHOD_WEIGHT && $carrier_obj->getMaxDeliveryPriceByWeight((int) $id_zone)
                            === false) || ($shipping_method == Carrier::SHIPPING_METHOD_PRICE
                            && $carrier_obj->getMaxDeliveryPriceByPrice((int) $id_zone)
                            === false)) {
                    ...
                    if ($total_product_price >= (float) ($free_fees_price) && (float) ($free_fees_price)
                            > 0) {
                        $shipping_cost                                          = 0;
                        $available_carriers[$pid]['shipping'][$key]['is_price'] = 0;
                        continue;
                    ...
                    if (isset($configuration['PS_SHIPPING_FREE_WEIGHT']) && $total_product_weight
                            >= (float) $configuration['PS_SHIPPING_FREE_WEIGHT']
                            && (float) $configuration['PS_SHIPPING_FREE_WEIGHT']
                            > 0) {
                        $shipping_cost                                          = 0;
                        $available_carriers[$pid]['shipping'][$key]['is_price'] = 0;
                    ...
                    if ($carrier['range_behavior']) {
                        ...
                        if (($shipping_method == Carrier::SHIPPING_METHOD_WEIGHT
                                && !$check_delivery_price_by_weight) || ($shipping_method
                                == Carrier::SHIPPING_METHOD_PRICE && !$check_delivery_price_by_price)) {
                            unset($available_carriers[$pid]['shipping'][$key]);
                            continue;
                        } else {
                            if ($shipping_method == Carrier::SHIPPING_METHOD_WEIGHT) {
...
//                    }
                    ...
                    $available_carriers[$pid]['shipping'][$key]['carrier_price']
                            = $shipping_cost;
                    $available_carriers[$pid]['shipping'][$key]['display_carrier_price']
                            = Tools::displayPrice($shipping_cost, (int) Context::getContext()->cart->id_currency);
                }
            }
        }
        ...
        foreach ($total_carriers as $pid => $carriers_list) {
            ...
            if ($product->is_virtual) {
                ...
                foreach ($carriers_list['shipping'] as $carrier) {
                    if (isset($selected_shipping[$pid]) && $carrier['id_carrier']
                            == $selected_shipping[$pid] && $carrier['is_price'] == 1) {
