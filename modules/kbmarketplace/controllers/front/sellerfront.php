...
class KbmarketplaceSellerfrontModuleFrontController extends KbmarketplaceFrontCoreModuleFrontController
{
    ...
    public function setMedia()
    {
        ...
        if (Tools::getIsset('render_type') && (Tools::getValue('render_type') == 'sellerview'
    ...
    public function initContent()
    {
        if (Tools::getIsset('render_type')) {
            ...
            } elseif (Tools::getValue('render_type') == 'sellerreviews') {
    ...
    public function renderSellerList()
    {
        ...
        if (Tools::getValue('searchcoupon') && Tools::getIsset('searchcoupon')) {
            ...
            if (!empty($coupon) && Tools::getIsset('coupon')) {
        ...
        if (Tools::isSubmit('new_review_submit') && Tools::getValue('new_review_submit')
            == 1) {
        ...
        $start = 1;
        if (Tools::getIsset('kb_page_start') && (int) Tools::getValue('kb_page_start')
            > 0) {
        ...
        if ($total > 0) {
            ...
            foreach ($sellers as $key => $val) {
                ...
                if ((int) $val['total_review'] > 0) {
                    ...
                    $sellers[$key]['rating_percent'] = (float) ((($val['rating']
                        / $tmp) / 5) * 100);
                } else {
    ...
    protected function processKbReport()
    {
        ...
        if (!empty($record)) {
            ...
            if (!empty($id_customer)) {
                ...
                $seller_product_review = KbSellerProductReview::getReviewsBySeller($id_seller, $id_lang);
                $customer_product_review = array();
                if (!empty($seller_product_review)) {
                    foreach ($seller_product_review as $sell_product_review) {
                        if ($sell_product_review['id_customer'] == $customer->id) {
//                            $sell_product_review['content'] = Tools::htmlentitiesDecodeUTF8($sell_product_review['content']);
                            $customer_product_review[] = $sell_product_review;
                        }
                    }
                }
            ...
            if ($seller->isSeller()) {
                ...
                */
                ...
                if (!empty($seller_info['id_country'])) {
                    $country_name = Country::getNameById(
                        $this->context->language->id,
    ...
    protected function saveNewReview($id_seller)
    {
        if (Configuration::get('KB_MP_ENABLE_SELLER_REVIEW') != 1) {
            $this->context->cookie->__set(
                'redirect_error',
                $this->module->l('Can not save the review as the option is disabled by admin', 'sellerfront')
            );
            return;
        }
        ...
        $approved = KbSellerSetting::getSellerSettingByKey(
            $id_seller,
            'kbmp_seller_review_approval_required'
        );
        if ($approved == 1) {
    ...
    protected function sendNewReviewMail($id_seller, $approved)
    {
        ...
        $template_vars = array(
            ...
            '{{review_comment}}' => Tools::getValue('review_content'),
        );
