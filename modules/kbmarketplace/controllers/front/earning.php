...
class KbmarketplaceEarningModuleFrontController extends KbmarketplaceCoreModuleFrontController
{
    ...
    {
        // changes by rishabh jain to include earning from cancelled orders
        $total_revenue = KbSellerEarning::getTotalEarningInSellerOrders($this->seller_info['id_seller'], true);
        $this->context->smarty->assign('total_revenue', $total_revenue);

        $total_earning = KbSellerEarning::getTotalSellerEarning($this->seller_info['id_seller'], true);
        ...
        if ($this->total_records > 0) {
            ...
            foreach ($seller_orders as $so) {
                ...
                $this->table_content[] = array(
                    ...
                    array('value' => Tools::displayPrice($so['seller_earning'], $currency),
                        'align' => 'kb-tright'),
                );
            }
    ...
    protected function getEarningHistoryList(
        ...
        // changes by rishabh jain to include the earning of cancelled order as well
    ...
    protected function getAjaxEarningHistoryHtml()
    {
        ...
        $format = ((array_key_exists($format, $this->getAllReportFormat())) ? $format
                    : self::REPORT_FORMAT_DAILY);
