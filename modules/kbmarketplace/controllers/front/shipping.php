...
class KbmarketplaceShippingModuleFrontController extends KbmarketplaceCoreModuleFrontController
{
    ...
    private function renderShippingForm()
    {
        ...
        if ($show_form) {
            ...
            $tax_group[] = array(
                'id' => 0,
                'name' => $this->module->l('No Tax', 'product')
            );
    ...
    protected function getTplRangesVarsAndValues($carrier)
    {
        ...
        foreach ($price_by_range as $price) {
            $tpl_vars['price_by_range'][$price['id_'.$range_table]][$price['id_zone']]
                = $price['price'];
        }
        ...
        if ($id_carrier != 0 && !KbSellerShipping::isSellerShipping($this->seller_obj->id, $id_carrier)) {
            ...
            if ($id_carrier > 0) {
                ...
                if (Validate::isLoadedObject($new_carrier)) {
                    ...
                    if ($error_mesg = $new_carrier->validateFieldsLang(false, true)
                        !== true) {
                        $return['has_error'] = true;
                        $return['errors'][]  = $error_mesg;
                    }
                ...
                if ($error_mesg                 = $carrier->validateFields(false, true)
                    !== true) {
                    $return['has_error'] = true;
                    $return['errors'][]  = $error_mesg;
                }
            ...
            if (Validate::isLoadedObject($carrier)) {
                ...
                foreach ($zones as $zone) {
                    if (count($carrier->getZone($zone['id_zone']))) {
                        if (!Tools::getIsset('zone_'.$zone['id_zone']) || !Tools::getValue('zone_'.$zone['id_zone'])) {
                            $success = $carrier->deleteZone((int) $zone['id_zone']);
                        }
                    } elseif (Tools::getIsset('zone_'.$zone['id_zone']) && Tools::getValue('zone_'.$zone['id_zone'])) {
                ...
                if (Tools::getValue('logo')) {
                    if (Tools::getValue('logo') == 'null' && file_exists(_PS_SHIP_IMG_DIR_.$carrier->id.'.jpg')) {
                        ...
                        /* Start - Code Modified by RS on 25-Aug-2017 for fixing 'In Shipping -> Add new Shipping, when user browse logo and click on remove then when user click on Save button then error messge is displayed.' issue (The control should not go inside the second condition if value is null as the log is already removed) */
                        if (Tools::getValue('logo') != 'null') {
                        ...
                        /* End - Code Modified by RS on 25-Aug-2017 for fixing 'In Shipping -> Add new Shipping, when user browse logo and click on remove then when user click on Save button then error messge is displayed.' issue (The control should not go inside the second condition if value is null as the log is already removed) */
                    }
                }
                ...
                if (is_array($fees) && count($fees)) {
                    foreach ($fees as $id_zone => $fee) {
                        $price_list[] = array(
                            ...
                            'id_zone' => (int) $id_zone,
                            'price' => isset($fee[$key]) ? (float) str_replace(',', '.', $fee[$key])
                                    : 0,
                        );
                    }
                }
...
//                    ),
            ...
            $this->kb_multiaction_params['multiaction_values'] = array(
                array('label' => $this->module->l('Map', 'shipping'), 'value' => $id_reference)
            );
            ...
            foreach ($results as $val) {
                ...
                $row_html .= '<tr>
                    ...
                    .$product->id.','.$this->seller_carrier->id_reference.')">
                            <i class="icon-retweet kb-multiaction-icon"></i>
							</a></td>
						</tr>';
            }
