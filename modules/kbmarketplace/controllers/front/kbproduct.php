...
class KbmarketplaceKbproductModuleFrontController extends KbmarketplaceCoreModuleFrontController
{
            ...
            if (Tools::isSubmit('method')) {
                switch (Tools::getValue('method')) {
                    ...
                    case 'getAjaxSubCategoryTree':
                        ...
                        die;
                    // changes by rishabh jain added optio to add new supplier
                    case 'saveSupplier':
            ...
            if ($id_product == 0
                ...
                if (KbSellerProduct::isSellerProduct($this->seller_info['id_seller'], $id_product)) {
                    if (Tools::getIsset('duplicateProduct')
                        && Tools::getValue('duplicateProduct') == 1
                    ) {
                        $this->processDuplicate();
                    } elseif (Tools::getIsset('deleteProduct')
    ...
    public function renderProductForm()
    {
        ...
        // Removed the Below Condition By Ashish on 3rd Fed 2018 as its not required
        ...
        if ($id_product == 0) {
            ...
            $product_limit = (int) KbSellerSetting::getSellerSettingByKey(
                $this->seller_obj->id,
                'kbmp_product_limit'
            );
                    ...
                    if (!empty($combinations)) {
                        foreach ($combinations as $combination) {
                            ...
                            if (!empty($combination['reference'])) {
                                ...
                                $results[$tmp1]['ref'] = !empty($item['reference'])
                                        ? $item['reference'] : '';
                            }
...
//        }
    ...
    public function processAddProductImage()
    {
        ...
        foreach ($files as &$file) {
            ...
            if (!$image->add()) {
                ...
                if ($image->legend[(int) $this->seller_info['id_default_lang']] == null) {
                    $leg = '';
                } else {
                    $leg = $image->legend[(int) $this->seller_info['id_default_lang']];
                }
                $file['legend']   = $leg;
    ...
    public function processDeleteImage()
    {
        ...
        if ($res) {
            return array(
                'status' => true,
                'msg' => $this->module->l('Image successfully deleted.', 'kbproduct')
            );
        } else {
            return array(
                'status' => true,
                'msg' => $this->module->l('Error occurred while deleting image.', 'kbproduct')
            );
        }
    }
    ...
    public function initFormQuantities()
    {
        ...
        if ($this->kb_product->id == null) {
            // When 2 then Out Of Stock is set as default value
            $out_of_stock_bit = 2;
        } else {
            $out_of_stock_bit = StockAvailable::outOfStock((int) $this->kb_product->id);
        }
        $this->context->smarty->assign('out_of_stock', $out_of_stock_bit);
    ...
    public function processSaveCombination()
    {
        ...
        $product = new Product($id_product);
        $errors = array();
        $attribute_combination_list = explode(
            ',',
        ...
        if (!count($errors)) {
            ...
            if (($id_product_attribute = (int) Tools::getValue('id_product_attribute'))
                ...
                /* changes done by rishabh jain on 16/10 2018 to add impacted weight and impacted price field */
    ...
    public function initFormVirtualProduct()
    {
        ...
        if ($id_product_download = $product_download->getIdFromIdProduct($this->kb_product->id)) {
            $product_download = new ProductDownload($id_product_download);
        }

        $this->kb_product->{'productDownload'} = $product_download;

        if ($this->kb_product->productDownload->id
            && empty($this->kb_product->productDownload->display_filename)
        ) {
            $this->errors[] = Tools::displayError($this->module->l('A file name is required in order to associate a file.', 'kbproduct'));
        ...
        if (!Tools::file_exists_no_cache($exists_file)
            ...
            $msg = Tools::displayError(sprintf($this->module->l('File "%s" is missing', 'kbproduct')), $this->kb_product->productDownload->display_filename);
        } else {
            $msg = '';
        }
        ...
        $this->kb_product->productDownload->nb_downloadable = ($this->kb_product->productDownload->id > 0)
            ...
            : htmlentities(
                Tools::getValue('virtual_product_nb_downloable'),
                ENT_COMPAT,
                'UTF-8'
            );
        ...
        $this->kb_product->productDownload->date_expiration = ($this->kb_product->productDownload->id > 0)
            ...
            && $this->kb_product->productDownload->date_expiration != '0000-00-00 00:00:00')
                    ...
                    : '' ) : htmlentities(
                        Tools::getValue('virtual_product_expiration_date'),
                        ENT_COMPAT,
                        'UTF-8'
                    );
        ...
        $this->kb_product->productDownload->nb_days_accessible = ($this->kb_product->productDownload->id > 0)
            ...
            : htmlentities(
                Tools::getValue('virtual_product_nb_days'),
                ENT_COMPAT,
                'UTF-8'
            );
    ...
    public function initFormPack()
    {
        ...
        if (Tools::getValue('namePackItems')) {
            ...
            $this->kb_product->packItems = Pack::getItems(
                $this->kb_product->id,
                $this->default_form_language
            );
        ...
        if (!$product) {
            ...
            foreach ($this->kb_product->packItems as $pack_item) {
                ...
                $pack_items[$i]['id_product_attribute'] = isset($pack_item->id_pack_product_attribute)
                    ...
                    ? $pack_item->id_pack_product_attribute : 0;
    ...
    public function processAdd()
    {
        ...
        if (!$this->seller_obj->isApprovedSeller()
            || $this->seller_obj->active == 0) {
            $object->active = 0;
        }
        ...
        if ($object->add()) {
            ...
            if (!$this->seller_obj->isApprovedSeller() || $this->seller_obj->active
                == 0) {
            ...
            if (Tools::getIsset('type_product') && Tools::getValue('type_product')
                == Product::PTYPE_PACK) {
                $this->updatePackItems($object);
            }

            if (Tools::getIsset('type_product')
                && Tools::getValue('type_product') == Product::PTYPE_VIRTUAL
            ) {
                $object->is_virtual = 1;
                $object->save();
            ...
            if (empty($this->Kberrors)) {
                ...
                if (Configuration::get('PS_DEFAULT_WAREHOUSE_NEW_PRODUCT') != 0
                    ...
                    $warehouse_location_entity->id_warehouse = Configuration::get(
                ...
                if (empty($this->Kberrors)) {
                    //BOC added by S Anand to resolve make products inactive if it is not approved yet
                    $seller_product = Db::getInstance()->getRow(
                        'SELECT * FROM ' . _DB_PREFIX_ . 'kb_mp_seller_product_tracking'
                        . ' WHERE id_product=' . (int) $object->id
                    );
                    /* Start - Code Modified by Raghu on 25-Aug-2017 for fixing 'Product enable/disable issue from seller account' issue */
                    if (is_array($seller_product) && count($seller_product) > 0) {
                        ...
                        Db::getInstance()->execute(
                            ...
                            . ' SET active=0,redirect_type="404" '
                            . 'WHERE id_shop='.(int) $this->seller_info['id_shop'] .' AND id_product='. (int)$object->id
                        );
                    }
                    /* Start - Code Modified by Raghu on 25-Aug-2017 for fixing 'Product enable/disable issue from seller account' issue */
                    //EOC added by S Anand to resolve make products inactive if it is not approved yet
    ...
    public function processUpdate()
    {
        ...
        if (isset($id) && !empty($id)) {
            ...
            if (Validate::isLoadedObject($object)) {
                ...
                /* End - Code Modified by Raghu on 25-Aug-2017 for fixing 'Product enable/disable issue from seller account' issue */
                #added by Mayank Bhaisora
                $object->id_tax_rules_group = $id_tax_rules_group;
                //added by mayank kumar

                if ($object->update()) {
                    if (Tools::getIsset('active') && Tools::getValue('active') == 0) {
                        $seller_product = Db::getInstance()->getRow(
                            'SELECT * FROM ' . _DB_PREFIX_ . 'kb_mp_seller_product_tracking'
                            . ' WHERE id_product=' . (int) $object->id
                        );
                    ...
                    } elseif ($after_status != $before_status) {
                        $seller_product = Db::getInstance()->getRow(
                            'SELECT * FROM ' . _DB_PREFIX_ . 'kb_mp_seller_product_tracking'
                            . ' WHERE id_product=' . (int) $object->id
                        );
                        if (empty($seller_product) || !$seller_product) {
                            Db::getInstance(_PS_USE_SQL_SLAVE_)->insert(
                                'kb_mp_seller_product_tracking',
                                array(
                                    'id_seller' => (int) $this->seller_info['id_seller'],
...
//                        $this->updateProductSuppliers($id, $id_suppliers);
                    ...
                    if (empty($this->Kberrors)) {
                        ...
                        if (Tools::getIsset('duplicateProduct') && Tools::getValue('duplicateProduct')) {
                            $this->context->cookie->__set(
                                'redirect_success',
                                sprintf($this->module->l('Product is duplicated successfully. Please change the SKU of Product ID #%s', 'kbproduct'), Tools::getValue('id_product')) .' .'
                            );
                        } else {
                                $this->context->cookie->__set(
                                    'redirect_success',
                        ...
                        if (is_array($seller_product) && count($seller_product) > 0) {
                            ...
                            Db::getInstance()->execute(
                                'UPDATE '._DB_PREFIX_.'product_shop'
                                . ' SET active=0,redirect_type="404" '
                                . 'WHERE id_shop='.(int) $this->seller_info['id_shop'] .' AND id_product='. (int)$id
                            );
                        }
                        /* End - Code Modified by Raghu on 25-Aug-2017 for fixing 'Product enable/disable issue from seller account' issue */
                        //EOC added by S Anand to resolve make products inactive if it is not approved yet
        ...
        $request_param = array();
        if (Tools::isSubmit('submitType')
            && Tools::getValue('submitType') == 'savenstay'
        ) {
    ...
    private function updateProductSuppliers($id_product, $id_suppliers = array(), $id_product_attribute = 0)
    {
        ...
        Db::getInstance()->delete('product_supplier', "id_product = $id_product");
...
//                    }
                    if ($field == 'description_short') {
                        $short_description_limit = Configuration::get('PS_PRODUCT_SHORT_DESC_LIMIT')
                                ? Configuration::get('PS_PRODUCT_SHORT_DESC_LIMIT') : 400;
                        // strip tags removed by rishabh jain
        ...
        $_POST['width']  = (!Tools::getIsset('width')) ? '0'
            : str_replace(',', '.', Tools::getValue('width'));
        $_POST['height'] = (!Tools::getIsset('height')) ? '0'
            : str_replace(',', '.', Tools::getValue('height'));
        $_POST['depth']  = (!Tools::getIsset('depth')) ? '0'
            : str_replace(',', '.', Tools::getValue('depth'));
        $_POST['weight'] = (!Tools::getIsset('weight')) ? '0'
            : str_replace(',', '.', Tools::getValue('weight'));
        ...
        // changes by rishabh jain to add/update custom fields
                    ...
                    if (!empty($line)) {
                        ...
                        if ($qty > 0 && isset($item_id)) {
                            if (Pack::isPack((int) $item_id)) {
                                $this->Kberrors[] = $this->module->l('You can\'t add product packs into a pack', 'kbproduct');
    ...
    {
        //check for product limit
        //Remvoed the First Condition By Ashish on 3rd Fed 2018 As its not required. Limit will be checked in all conditions.
        if (!$this->seller_obj->isApprovedSeller() || $this->seller_obj->active == 0) {
            $added_product_count = (int) $this->seller_obj->product_limit_wout_approval;
            $product_limit = (int) KbSellerSetting::getSellerSettingByKey(
                $this->seller_obj->id,
        ...
        if (Validate::isLoadedObject($product = new Product((int) Tools::getValue('id_product')))) {
            ...
            if ($product->add()) {
                ...
                $seller_product->deleted    = 0;
                if ($this->seller_info['settings']['kbmp_new_product_approval_required']
                    == 0) {
                    $seller_product->approved = (string) KbGlobal::APPROVED;
                } else {
                    $seller_product->approved = (string) KbGlobal::APPROVAL_WAITING;
                ...
                if (Category::duplicateProductCategories($id_product_old, $product->id)
                    && ($combination_images = Product::duplicateAttributes($id_product_old, $product->id))
                    !== false && GroupReduction::duplicateReduction($id_product_old, $product->id)
    ...
    private function sendNotificationForProductApproval($product)
    {
        ...
        $data = array(
            ...
            'title' => (!empty($this->seller_info['title'])) ?
            $this->seller_info['title'] : 'Anonymous seller',
    ...
    public function processDelete()
    {
        if (Validate::isLoadedObject($object = new Product((int) Tools::getValue('id_product')))) {
            ...
            if (Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT')
                && $object->advanced_stock_management
            ) {
                ...
                if ($physical_quantity > 0 || $real_quantity > $physical_quantity) {
                    $this->Kberrors[] = $this->module->l('You cannot delete this product because there\'s physical stock left.', 'kbproduct');
                }
            }
    ...
    public function getAjaxProductListHtml()
    {
        ...
        if ($this->total_records > 0) {
            ...
            foreach ($results as $val) {
                // changes by rishabh jainfor product image
                // changes by rishabh jain
                $sql = 'SELECT SQL_CALC_FOUND_ROWS 
                        sav.`quantity`  AS `sav_quantity`
                        FROM  ' . _DB_PREFIX_ . 'product p 
                ...
                $row_html .= '<tr>
                    ...
                    .'" class="btn btn-default kb-multiaction-link">
                            <i class="icon-edit kb-multiaction-icon"></i>
                            </a>
                            <a href="javascript:void(0)"
                        title="'.$this->module->l('Click to delete product', 'kbproduct')
                    .'" class="btn btn-default kb-multiaction-link" onclick="KbDeleteAction('
                    .$product->id.')"><i class="icon-trash kb-multiaction-icon"></i>
                    </a>
                    </tr>';
            }
    ...
    public function processMultiAction()
    {
        ...
        if (Tools::getIsset('mutiaction_type')
            ...
            if (Tools::getIsset('mutiaction_status_list')) {
                ...
                if (!$all_updated) {
                    $this->context->cookie->__set(
                        'redirect_success',
                        sprintf($this->module->l('<b>%s</b> product(s) has been updated out of <b>%s</b> product(s).', 'kbproduct'), $update_count, count($product_ids))
                    );
                } else {
                    $this->context->cookie->__set(
                        'redirect_success',
        ...
        } elseif (Tools::getIsset('mutiaction_type') && Tools::getValue('mutiaction_type')
            == KbGlobal::MULTI_ACTION_TYPE_DELETE) {
            $product_ids   = explode(',', trim(Tools::getValue('selected_table_item_ids')));
            $cannot_delete = 0;
            foreach ($product_ids as $id) {
                if ((int) $id > 0) {
            ...
            if ($cannot_delete > 0) {
                if (!$all_updated) {
                    $this->context->cookie->__set(
                        'redirect_success',
                        sprintf(
                            $this->module->l('<b>%s</b> product(s) has been deleted out of <b>%s</b> product(s) and <b>%s</b> product(s) cannot be delete due to physical stock.', 'kbproduct'),
                            $update_count,
                            count($product_ids),
                            $cannot_delete
                        )
                    ...
                    $this->context->cookie->__set(
                        'redirect_success',
                        sprintf(
                            $this->module->l('<b>%s</b> product(s) has been deleted successfully and <b>%s</b> product(s) cannot be delete due to physical stock.', 'kbproduct'),
                            $update_count,
                            $cannot_delete
                        )
                    );
    ...
    public function processProductFeatures($product_id)
    {
        ...
        if (Validate::isLoadedObject($product = new Product((int)$product_id))) {
            ...
            $post_data = $_POST;
            foreach ($_POST as $key => $val) {
                if (preg_match('/^feature_([0-9]+)_cvalue/i', $key, $match)) {
                    $key2 = str_replace("cvalue", "value", $key);
                    if ($val) {
                        if (isset($post_data[$key2])) {
                            unset($post_data[$key2]);
                        }
                    }
                }
            }
