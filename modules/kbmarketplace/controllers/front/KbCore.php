...
class KbmarketplaceCoreModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        if ($this->seller_obj->approved == KbGlobal::DISSAPPROVED && $this->seller_obj->approval_request_limit
            > 0) {
        ...
        if ($layout) {
            ...
            Tools::displayAsDeprecated(
                sprintf($this->module->l('Market Place layout file is missing from %s directory.', 'kbcore'), $this->getKbTemplateDir())
            );
        }
    }
    ...
    protected function setKbTemplate($template)
    {
        if (!$path = $this->getKbTemplatePath($template)) {
            throw new PrestaShopException(sprintf($this->module->l('Template %s not found', 'kbcore')), $template);
        }
    ...
    public function generatePaginator($current_page, $total_records, $total_pages, $ajaxcallfn = '')
    {
        ...
        if ($total_pages > 0 && $total_pages != 1 && $current_page <= $total_pages) {
            ...
            if ($current_page > 1) {
                $record_start = (($current_page - 1) * (int) $this->tbl_row_limit)
                    + 1;
    ...
    protected function getFieldValue($obj, $key, $id_lang = null, $default = "")
    {
        $default_value = false;
        if ($id_lang) {
