    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('ajax')) {
            ...
            if (Tools::isSubmit('updateSellerProfile')) {
                if (Tools::getValue('updateSellerProfile') != Tools::encrypt($this->controller_name.$this->seller->id)
                ) {
                    ...
                    if ((isset($_FILES['seller_logo']) && $_FILES['seller_logo']['size']
                        > 0) || Tools::getValue('seller_logo_update')) {
                    ...
                    if ((isset($_FILES['seller_banner']) && $_FILES['seller_banner']['size']
                        > 0) || Tools::getValue('seller_banner_update')) {
                        if ((int) Tools::getValue('seller_banner_update') > 0) {
                            $this->deleteImage($seller_img_path.$this->seller->id.'-banner'.$this->imageType);
                        }
                        if (isset($_FILES['seller_banner'])
                            && $_FILES['seller_banner']['size'] > 0
                            && $this->uploadImage(
                                'seller_banner',
                                $seller_img_path,
                    ...
                    if (!empty($validate_fields)) {
                        ...
                        if ($this->seller->save(true)) {
                            /*
                             * Start- MK made changes on 29-06-18 to update the seller data for the other language if in that language data is empty
                             */
                            foreach ($languages as $lang) {
                                ...
                                if (!empty($result) && count($result) >= 1) {
                                    DB::getInstance()->execute(
                                        ...
                                        . 'meta_keyword="' . pSQL($seller_meta_keyword_lang) . '",'
                                        . 'meta_description="' . pSQL($seller_meta_desc_lang) . '",'
                                        . 'profile_url="' . pSQL($seller_url_lang) . '",'
                                        . 'return_policy="' . pSQL($seller_return_lang, true) . '",'
                                        . 'shipping_policy="' . pSQL($seller_shipping_lang, true) . '",'
                                        . 'return_address="' . pSQL($seller_return_addr) . '",'
...
//                                                . 'return_address="' . pSQL($this->seller->return_address)
//                                                . '" WHERE id_seller_lang=' . (int) $result['id_seller_lang']
//                                            );
//                                        }
//                                    }
...
//                            chmod(_PS_MODULE_DIR_ .  'kbmarketplace/views/upload/' . time() . '.' . $file_extension, 0777);
    ...
    public function initContent()
    {
        ...
        $this->context->smarty->assign(array('meta_title' => $this->module->l('Seller Profile', 'seller')));
        $this->context->smarty->assign(array('meta_description' => $this->seller->meta_description));
        $this->context->smarty->assign(array('meta_keywords' => $this->seller->meta_keyword));
        ...
        $this->context->smarty->assign('countries_enable', $country_array);

        if ((int) $this->seller->id_country > 0) {
            $seller_country = (int) $this->seller->id_country;
        } elseif (Tools::getIsset('seller_country') && (int) Tools::getValue('seller_country')
            > 0) {
