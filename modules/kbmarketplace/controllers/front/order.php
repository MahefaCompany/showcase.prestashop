...
//            }
//            die;
//        } elseif (Tools::getIsset('submitState') && $id_order = Tools::getValue('id_order', 0)) {
...
//        }
    ...
    public function initContent()
    {
        ...
        } elseif (Tools::getIsset('render_type') && Tools::getValue('render_type')
            == 'print_order') {
        ...
        if (KbSellerEarning::isSellerOrder($id_seller, $id_order)) {
            ...
            if (Validate::isLoadedObject($order)) {
                        ...
                        if (KbSellerProduct::isSellerProduct($id_seller, $detail['product_id'])) {
                            if ($detail['image'] != null) {
                                $name = 'product_mini_'.(int) $detail['product_id']
                                    .(isset($detail['product_attribute_id']) ? '_'.(int) $detail['product_attribute_id']
                                            : '').'.jpg';
                ...
                foreach ($history as &$order_his) {
                    $order_his['text-color'] = Tools::getBrightness($order_his['color'])
                        < 128 ? 'white' : 'black';
                }
                $setting_mark = Tools::unserialize(Configuration::get('KB_MARKETPLACE_CONFIG'));
                $message = CustomerMessage::getMessagesByOrderId((int)($order->id), 0);
//                print_r($message);
//                die;
...
//                d($setting_mark);
    ...
    private function renderList()
    {
        ...
        foreach ($tmp as $val) {
            $statuses[$val['id_order_state']] = array('value' => $val['id_order_state'],
                'label' => $val['name']);
        }
        ...
        if ($this->total_records > 0) {
            ...
            foreach ($seller_orders as $so) {
                ...
                $this->table_content[] = array(
                    ...
                    array('value' => Tools::displayPrice($so['total_earning'], $currency),
                        'align' => 'kb-tright')
                );
            }
        ...
        $total_revenue = Tools::displayPrice(
            ...
            // to show the earning of cancelled order as well
        ...
        if (KbSellerEarning::isSellerOrder($this->seller_obj->id, $id_order)) {
            ...
            if (!Validate::isLoadedObject($customer)) {
                ...
                foreach ($rules['required'] as $field) {
                    if (($value = Tools::getValue($field)) == false && (string) $value
                        != '0') {
                        if (!Tools::getValue('id_'.$this->table) || $field != 'passwd') {
                            $errors[] = sprintf($this->module->l('Field %s is required.', 'order'), $field);
                        }
                    }
                }
                foreach ($rules['size'] as $field => $maxLength) {
                    if (Tools::getValue($field) && Tools::strlen(Tools::getValue($field))
                        > $maxLength) {
                        $errors[] = sprintf(
                            $this->module->l('Field %1$s is too long (%2$d chars max).', 'order'),
                            $field,
                            $maxLength
                        );
                    }
                ...
                if (!count($errors)) {
                    ...
                    if (!$customer_message->add()) {
                        ...
                        if (!$customer_message->private) {
                            ...
                            if (Configuration::get('PS_MAIL_TYPE', null, null, $order->id_shop)
                                != Mail::TYPE_TEXT) {
