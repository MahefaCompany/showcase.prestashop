...
//                'havingFilter' => true,
        ...
        $this->_where .= ' AND a.approved IN ('. (int) KbGlobal::APPROVAL_WAITING.','. (int) KbGlobal::DISSAPPROVED.')';
        ...
        if (!empty($file) && is_array($file)) {
            if (isset($file['type'])) {
                ...
                if (Tools::file_exists_no_cache($file['path'])) {
                    ...
                    Tools::redirect(
                        $this->context->link->getAdminLink('AdminKbSellerApprovalList', true)
                    );
                }
            } else {
                Tools::redirect(
    ...
    public function processStatus()
    {
        ...
        if (Validate::isLoadedObject($object = $this->loadObject())) {
            ...
            if (!array_key_exists('active', $object)) {
                throw new PrestaShopException(sprintf($this->module->l('property "active" is missing in object %s', 'adminkbsellerapprovallistcontroller'), get_class($this)));
            }
            ...
            if ($is_update) {
                ...
                if (preg_match(
                    ...
                    $matches
                ) !== false && Tools::strtolower($matches[1]) != Tools::strtolower(preg_replace(
                    '/controller/i',
                    '',
                    get_class($this)
                ))) {
    ...
    public function renderView()
    {
        ...
        if (!Validate::isLoadedObject($seller_obj)) {
            ...
            $tpl_vars = array(
                'id_seller' => $id_seller,
                'module_path' => $this->context->link->getAdminLink('AdminKbSellerApprovalList', true),
    ...
    public function getPaymentMethodname($name)
    {
        $payment_methods = array(
            ...
            'check' => $this->module->l('Payment by Cheque', 'adminkbsellerapprovallistcontroller'),
