        ...
        $this->fields_form = array(
            ...
            'input' => array(
                ...
                array(
                    ...
                    'multiple' => true,
                    'class' => 'chosen',
                    'label' => $this->l('Order Status for cancellation'),
    ...
    public function initProcess()
    {
        if (Tools::isSubmit('submitMarketPlaceConfiguration')) {
            ...
            if ($this->tabAccess['edit'] === '1') {
                ...
                $error_txt = $this->module->l('You do not have permission to edit this.', 'adminkbmarketplacesettingcontroller');
                $this->errors[] = Tools::displayError($error_txt);
            }
        }
    }
                ...
                if (isset($field['lang']) && $field['lang']) {
                    ...
                    if (isset($field['required']) && $field['required']) {
                        if (($value = Tools::getValue($field['name'])) == false && (string) $value
                            != '0') {
                            $error          = true;
                            $this->errors[] = sprintf(Tools::displayError($this->module->l('Field %s is required.', 'adminkbmarketplacesettingcontroller')), $field['label']);
                        } elseif (isset($field['validation']) && !call_user_func(array(
                                "Validate", $field['validation']), Tools::getValue($field['name']))) {
                            $error          = true;
                            $this->errors[] = sprintf(Tools::displayError($this->module->l('Field %s is invalid.', 'adminkbmarketplacesettingcontroller')), $field['label']);
                        }
                    } elseif (isset($field['validation']) && !call_user_func(array(
                            "Validate", $field['validation']), Tools::getValue($field['name']))) {
                        $error          = true;
    ...
    public function getFieldsValue($obj)
    {
        ...
        if (!Configuration::get('KB_MARKETPLACE_CONFIG') || Configuration::get('KB_MARKETPLACE_CONFIG')
            ...
            if (isset($settings['kbmp_seller_order_email_template'])
                && $settings['kbmp_seller_order_email_template'][1] == '0'
            ) {
            ...
            if (isset($settings['kbmp_seller_order_cancel_email_template'])
                && $settings['kbmp_seller_order_cancel_email_template'][1] == '0'
            ) {
        ...
        foreach ($this->fields_form[0]['form']['input'] as $fieldset) {
            if (isset($fieldset['lang']) && $fieldset['lang']) {
                ...
                foreach ($this->_languages as $language) {
                    $lang_data[$language['id_lang']] = '';
                    if (Tools::getIsset($fieldset['name'].'_'.$language['id_lang'])) {
                ...
                if ($fieldset['type'] == 'select') {
                    if (isset($fieldset['multiple']) && $fieldset['multiple']) {
                        $this->fields_value[$fieldset['name']] = (array) $settings[$fieldset['name']];
                    } else {
