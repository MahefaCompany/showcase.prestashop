...
class AdminKbadminOrderListController extends AdminKbMarketplaceCoreController
{
    ...
    public function __construct()
    {
		...
		osl.`name` AS `osname`,
		os.`color`,
		IF((SELECT so.id_order FROM `'._DB_PREFIX_.'orders` so WHERE
                so.id_customer = a.id_customer AND so.id_order < a.id_order LIMIT 1) > 0, 0, 1) as new,
		...
		INNER JOIN `'._DB_PREFIX_.'country_lang` country_lang ON
                ...
                country_lang.`id_lang` = '.(int) $this->context->language->id.')
		LEFT JOIN `'._DB_PREFIX_.'order_state` os ON (os.`id_order_state` = a.`current_state`)
		LEFT JOIN `'._DB_PREFIX_.'order_state_lang` osl ON (os.`id_order_state` = osl.`id_order_state`
                AND osl.`id_lang` = '.(int) $this->context->language->id.')';
        $this->_where    = 'AND a.id_order NOT IN '
                . '(select sl.id_order from `'._DB_PREFIX_.'kb_mp_seller_earning` as sl)';
