...
//                'havingFilter' => true,
        ...
        $this->_join = '
			LEFT JOIN `'._DB_PREFIX_. pSQL($this->table).'_lang` sl ON (a.`id_seller` = sl.`id_seller`
        ...
        if (!empty($file) && is_array($file)) {
            if (isset($file['type'])) {
                ...
                if (Tools::file_exists_no_cache($file['path'])) {
                    ...
                    Tools::redirect(
                        $this->context->link->getAdminLink('AdminKbSellerList', true)
                    );
                }
            } else {
                Tools::redirect(
    ...
    public function initPageHeaderToolbar()
    {
        ...
        } elseif (!empty($this->display) && ($this->display == 'update' || $this->display
            == 'edit')) {
            $seller = $this->loadObject(true);
            if (Validate::isLoadedObject($seller)) {
                $seller_info = $seller->getSellerInfo();
    ...
    public function processStatus()
    {
        ...
        if (Validate::isLoadedObject($object = $this->loadObject())) {
            ...
            if (!array_key_exists('active', $object)) {
                throw new PrestaShopException(sprintf($this->module->l('Property "active" is missing in object %s', 'adminkbsellerlistcontroller'), get_class($this)));
            }
