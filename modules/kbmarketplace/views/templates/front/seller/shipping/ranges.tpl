    ...
    <tbody id="shipping_ranges_body">
        <tr class="range_inf">
            ...
            <td>
                <div class="kb-labeled-inpfield input-group">
                    <span class="inplbl weight_unit" style="display:{if $carrier->shipping_method == $shipping_method_weight}table-cell{else}none{/if};">{$ps_weight_unit|escape:'htmlall':'UTF-8'}</span>
                    <span class="inplbl price_unit" style="display:{if $carrier->shipping_method == $shipping_method_price}table-cell{else}none{/if};">{$ps_currency_unit|escape:'htmlall':'UTF-8'}</span>
                    <input class="kb-inpfield" validate="isPrice" name="range_inf[{$range.id_range|intval}]" type="text" value="{number_format($range.delimiter1, 6,'.','')|escape:'htmlall':'UTF-8'}" />
                </div>
            </td>
            {foreachelse}
            <td>
        ...
        <tr class="range_sup">
            ...
            <td class="range_data">
                <div class="kb-labeled-inpfield input-group">
                    <span class="inplbl weight_unit" style="display:{if $carrier->shipping_method == $shipping_method_weight}table-cell{else}none{/if};">{$ps_weight_unit|escape:'htmlall':'UTF-8'}</span>
                    <span class="inplbl price_unit" style="display:{if $carrier->shipping_method == $shipping_method_price}table-cell{else}none{/if};">{$ps_currency_unit|escape:'htmlall':'UTF-8'}</span>
                    <input class="kb-inpfield" name="range_sup[{$range.id_range|intval}]" type="text" value="{if isset($change_ranges) && $range.id_range == 0} {else}{number_format($range.delimiter2, 6,'.','')|escape:'htmlall':'UTF-8'}{/if}" autocomplete="off"/>
                </div>
            </td>
            {foreachelse}
            <td class="range_data">
        ...
        <tr class="fees_all">
            ...
            <td class=" {if $range.id_range != 0} validated {/if}">
                <div class="kb-labeled-inpfield input-group">
                    <span class="inplbl currency_sign"{if $range.id_range == 0} style="display:none" {/if}>{$ps_currency_unit|escape:'htmlall':'UTF-8'}</span>
            ...
            <td class="">
                <div class="kb-labeled-inpfield input-group">
                    <span class="inplbl" style="display:none">{$ps_currency_unit|escape:'htmlall':'UTF-8'}</span>
        ...
        {foreach from=$zones key=i item=zone}
            <tr class="fees" data-zoneid="{$zone.id_zone|intval}">
                ...
                <td>
                    <div class="kb-labeled-inpfield input-group">
                        <span class="inplbl" >{$ps_currency_unit|escape:'htmlall':'UTF-8'}</span>
                        ...
                        {if isset($price_by_range[$range.id_range][$zone.id_zone]) && $price_by_range[$range.id_range][$zone.id_zone] && isset($fields_value['zones'][$zone.id_zone]) && $fields_value['zones'][$zone.id_zone]} value="{number_format($price_by_range[$range.id_range][$zone.id_zone], 6,'.','')|escape:'htmlall':'UTF-8'}" {else} value="" {/if} />
                    </div>
                </td>
                {/foreach}
            </tr>
