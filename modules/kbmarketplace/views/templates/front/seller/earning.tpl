    ...
    <div class="kb_tab_container">
        <div id="tab1" class="kb_tab_content">
            {if isset($kb_earning_history_filter)}
                {$kb_earning_history_filter}{*Variable contains css and html content, escape not required*}
            {/if}

            {if isset($kb_earning_history_list)}
                <div class="kb-vspacer5"></div>
                {$kb_earning_history_list}{*Variable contains css and html content, escape not required*}
            {/if}
        </div>
        <div id="tab2" class="kb_tab_content">
            {if isset($kb_owise_earning_filter)}
                {$kb_owise_earning_filter}{*Variable contains css and html content, escape not required*}
            {/if}

            {if isset($kb_owise_earning_list)}
                <div class="kb-vspacer5"></div>
                {$kb_owise_earning_list}{*Variable contains css and html content, escape not required*}
            {/if}
        </div>
    </div>    
</div>
