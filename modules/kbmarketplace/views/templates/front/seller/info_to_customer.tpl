<div class="kb-block seller_profile_view">
    <div class="s-vp-banner">
        <img src="{$seller['banner']|escape:'htmlall':'UTF-8'}" />
    </div>
    <div class="info-view">
        <div class="seller-profile-photo">
            <a href="{$link->getModuleLink($kb_module_name, 'sellerfront', ['render_type' => 'sellerview', 'id_seller' => $seller['id_seller']], (bool)Configuration::get('PS_SSL_ENABLED'))|escape:'htmlall':'UTF-8'}" >
                <img src="{$seller['logo']|escape:'htmlall':'UTF-8'}" title="{$seller['title']|escape:'htmlall':'UTF-8'}" alt="{$seller['title']|escape:'htmlall':'UTF-8'}">
            </a>
        </div>
            ...
            <div class="seller-basic">
                <div class="seller-name">
                    ...
                    <div class="seller-rating-block">
                            <div class="kbmp-_inner_block"><i class="icon-heart shortlist_link" style="{if isset($is_already_added) && $is_already_added == 1}color: #ef4545;{else}color:grey;{/if}"></i></div>
                            <div class="kbmp-_inner_block" style="position:relative;">
                                <div class="vss_seller_ratings">
                                    <!-- Do not customise it -->
                                    <a href="javascript:addShortListSeller(this, {$seller['id_seller']|escape:'htmlall':'UTF-8'});" class="sfl_product_link_{$seller['id_seller']|escape:'htmlall':'UTF-8'}" style="padding-left:7px;font-size:13px;color: #2fb5d2;">{if isset($is_already_added) && $is_already_added == 1}{l s='Favourite Seller' mod='kbmarketplace'}{else}{l s='Mark Seller as Favourite' mod='kbmarketplace'}{/if}</a>
                        ...
                        {if !isset($seller['is_review_page'])}
                            ...
                            {if $display_new_review}
                                <div class="kbmp-_inner_block">
                                    ...
                                    {else}
                                        <a href="#kb-seller-new-review-popup" class="vss_active_link vss_write_review_bck open-slr-review-form"><span class="">{l s='Write Review' mod='kbmarketplace'}</span></a>
                                    {/if}
                                </div>
                            {/if}
                        {else}
                ...
                <div class="seller-social">
                    {if !empty($seller['twit_link'])}
                        <a title="{l s='Twitter' mod='kbmarketplace'}" href="{$seller['twit_link']|escape:'htmlall':'UTF-8'}"><i class="icon-twitter"></i></a>
                    {/if}
                    {if !empty($seller['fb_link'])}
                        <a title="{l s='Facebook' mod='kbmarketplace'}" href="{$seller['fb_link']|escape:'htmlall':'UTF-8'}"><i class="icon-facebook"></i></a>
                    {/if}
                    {if !empty($seller['gplus_link'])}
                        <a title="{l s='Google+' mod='kbmarketplace'}" href="{$seller['gplus_link']|escape:'htmlall':'UTF-8'}"><i class="icon-google-plus"></i></a>
                    {/if}       
                </div>
            </div>
        </div>
        ...
        {if !empty($seller['description'])}
            <section class="slr-f-box">
                ...
                <div  class="rte slr-content">
                    {html_entity_decode($seller['description'])}{*Variable contains HTML content,escape not required*}
                </div>
            </section>
        {/if}
         <!--  <section class="slr-f-box">
            ...
            <div  class="rte slr-content">
                {if !empty($seller['privacy_policy'])}
                    {html_entity_decode($seller['privacy_policy'])}{*Variable contains HTML content,escape not required*}
                {else}
        ...
        <section class="slr-f-box">
            ...
            <div  class="rte slr-content">
                {if !empty($seller['return_policy'])}
                    {html_entity_decode($seller['return_policy'])}{*Variable contains HTML content,escape not required*}
                {else}
      ...
      <section class="slr-f-box">
            ...
            <div  class="rte slr-content">
                {if !empty($seller['shipping_policy'])}
                    {html_entity_decode($seller['shipping_policy'])}{*Variable contains HTML content,escape not required*}
                {else}
...
{if isset($display_review_popup) && $display_review_popup}
    <div id="kb-seller-new-review-popup" style="display:none; width:400px">
        <div id='slr-new-review-blk' class='s-review-n'>
            ...
            <form id="slr-review-form" action="{$link->getModuleLink($kb_module_name, 'sellerfront', ['render_type' => 'sellerview', 'id_seller' => $seller['id_seller']], (bool)Configuration::get('PS_SSL_ENABLED'))|escape:'htmlall':'UTF-8'}" method="post">
                <ul>
                    <li>
...
{if $gdpr_enabled}
    <div id="kb-seller-access-data-popup">
        <div class="kb-popup-content">
            ...
            <form class="kb_mp_personal_access_form" action="{$filter_form_action|escape:'quotes':'UTF-8'}" method="post">
                <div class="form-group">
                    <label for="email">{l s='Email Address' mod='kbmarketplace'}</label>
                    <input type="hidden" name="id_seller" value="{$seller['id_seller']|escape:'htmlall':'UTF-8'}">
...
{/if}
    <script type="text/javascript">
            ...
            var seller_front_url = "{$filter_form_action|escape:'quotes':'UTF-8'}";
            var kb_email_not_exit= "{l s='Email Address is not associated with any account.' mod='kbmarketplace'}";
        </script>
        <script>
        var ajaxurl = "{$ajaxurl}"; {* Variable contains HTML/CSS/JSON, escape not required *}
