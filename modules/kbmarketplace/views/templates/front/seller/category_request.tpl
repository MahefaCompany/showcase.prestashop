            ...
            <form id='kb_category_request_form' action='' method='post'>
                <ul class="kb-form-list">
                        ...
                        <div class="kb-form-field-block">
                            <select name="category_request_id" class="kb-inpselect">
                                {foreach $available_categories as $cat}
                                    <option value="{$cat['id_category']|intval}" {if $request_category_id == $cat['id_category']}selected="selected"{/if}>{$cat['name']|escape:'htmlall':'UTF-8'}</option>
                                {/foreach}
                            </select>
                        </div>
                    </li>
                    <li class="kb-form-fwidth last-row">
                        ...
                        <div class="kb-form-field-block">
                            <textarea name="category_request_reason" rows="5" class="kb-inptexarea">{$request_category_reason|escape:'htmlall':'UTF-8'}</textarea>
                ...
                <div class='kb-vspacer5'></div>
                <button type="button" class='kbbtn-big kbbtn-success' onclick='submitCategoryRequest()'>{l s='Save' mod='kbmarketplace'}</button>
            </form>
        {else}
            <div class="kbalert kbalert-warning">{l s='Sorry! No unassigned category available for you.' mod='kbmarketplace'}</div>
        {/if}
        {addJsDefL name=cat_request_reason_min_length_error}{l s='Minimum %s characters are required for reason.' mod='kbmarketplace' sprintf=$min_reason_length js=1}{/addJsDefL}
        {addJsDefL name=cat_request_reason_required}{l s='Reason is required' mod='kbmarketplace' js=1}{/addJsDefL}
        {addJsDefL name=cat_request_reason_max_length_error}{l s='Maximum %s characters are allowed for reason.' mod='kbmarketplace' sprintf=$max_reason_length js=1}{/addJsDefL}
        {addJsDef cat_request_reason_min_length=$min_reason_length|intval}
        {addJsDef cat_request_reason_max_length=$max_reason_length|intval}
    </div>
    <div class="kb-content-header">
