    ...
    <div id="kb-product-form-combination" class='kb-panel-body'>
        {if $id_product eq 0}
            <div class="kbalert kbalert-warning pack-empty-warning">
                <i class="icon-exclamation-sign" style="font-size:12px; margin-right:5px;"></i> {l s='Before creating combination, you must have to save this product.' mod='kbmarketplace'}
            </div>
        {else}
                ...
                <ul class="kb-form-list">
                    <li class="kb-form-fwidth last-row" style='min-height:inherit;'>
                        <div class="kb-form-field-inblock" style="float:right;">
                            <a href="javascript:void(0)" class="kbbtn btn-info" onclick="editCombination(0);"><i class="icon-add"></i><span>{l s='Add New Combination' mod='kbmarketplace'}</span></a>
                        </div>
                        <div class="clearfix"></div>
                    </li>
                    <li class="kb-form-fwidth last-row" style='overflow-y: auto'>
                        <div id="combination-section-msg" class="kbalert kbalert-info" style="display:{if count($attributes) > 0}block{else}none{/if};"><i class="icon-exclamation-sign" style="font-size: 13px;"></i>{l s='Blue row indicates the default combination.' mod='kbmarketplace'}</div>
                        ...
                        <table class="kb-table-list" style="display:none;">
                            <tbody id="new_combination_template">
                                <tr id="id_product_attribute">                        
                                    ...
                                    <td class="kb-tcenter">
                                        <button type="button" class="btn btn-default" onclick="editCombination(id_product_attribute);" title="{l s='Click to edit' mod='kbmarketplace'}"><i class="icon-edit"></i></button>
                                        <button type="button" class="btn btn-default" onclick="deleteCombination(id_product_attribute);" title="{l s='Click to delete' mod='kbmarketplace'}"><i class="icon-trash"></i></button>
                                    </td>
                                </tr>    
                            </tbody>
                        </table>
...
{addJsDefL name=kb_no_combination_msg}{l s='No Combination available for this product.' mod='kbmarketplace' js=1}{/addJsDefL}
{addJsDefL name=kb_new_combination_title}{l s='Add New Combination' mod='kbmarketplace' js=1}{/addJsDefL}
{addJsDefL name=kb_edit_combination_title}{l s='Edit Combination' mod='kbmarketplace' js=1}{/addJsDefL}
{addJsDefL name=kb_edit_combination_yes}{l s='Yes' mod='kbmarketplace' js=1}{/addJsDefL}
{addJsDefL name=kb_edit_combination_no}{l s='No' mod='kbmarketplace' js=1}{/addJsDefL}
        ...
        {if count($attributes) > 0}
            {foreach $attributes as $attribute}
                displayCombinationRow({$attribute['id_product_attribute']|intval},
                    ...
                    "{$attribute['default_image']}");{*Variable contains css and html content, escape not required*}
            {/foreach}
        {else}
            $('#kb_product_combination_list').html('<tr><td colspan="8" class="kb-empty-table kb-tcenter">'+kb_no_combination_msg+'</td></tr>');
        {/if}
    });
</script>
<div id="kb-combination-modal-form" style="display:none;">
    ...
    <div id="combination-form-content" class="kb-modal" style="display:none">
            ...
            <div id="new-combination-form" class="new_cmbination_form kb-form" style="padding:0;">
                <ul class="kb-form-list">
                    <li class="kb-form-fwidth">
                        ...
                        <div class="kb-form-field-block">
                            ...
                            <div class="form-field-indis">
                                <button type="button" class="btn btn-info" onclick="add_attr();"><i class="icon-plus" ></i> {l s='Add' mod='kbmarketplace'}</button>
                            </div>
                        </div>
                    </li>
                    <li class="kb-form-fwidth">
                        <div class="kb-form-field-block kb-mbtm10">
                            ...
                            <div class="form-field-block">
                                <select id="product_att_list" name="attribute_combination_list[]" multiple="multiple" class="kb-inpselect"></select>
                            </div>    
                        </div>
                        <div class="kb-form-field-block">
                            <button type="button" class="btn btn-danger" onclick="del_attr()"><i class="icon-trash"></i> {l s='Delete' mod='kbmarketplace'}</button>
                        </div>
                    </li>
                    <li class="kb-form-fwidth">
                    ...
                    <li class="kb-form-fwidth" style="border-bottom:0;">
                        <div class="kb-form-label-block">
                            <span class="kblabel"><i class="icon-question" data-toggle="tooltip" data-placement="top" data-original-title="{l s='The next date of availability for this product when it is out of stock.' mod='kbmarketplace'}"></i>{l s='Available Date' mod='kbmarketplace'}</span>
                        </div>
                        <div class="kb-form-field-block">
        ...
        <div class="kb-modal-footer">
            <button id="combination-submit" type="button" class="kbbtn-big kbbtn-success" data-id="0" onclick="saveCombination()">{l s='Submit' mod='kbmarketplace'}</button>
