    ...
    <div id="kb-product-form-returnpolicy" class='kb-panel-body'>
        {if $id_product eq 0}
            <div class="kbalert kbalert-warning pack-empty-warning">
                <i class="icon-exclamation-sign" style="font-size:12px; margin-right:5px;"></i>{l s='Before mapping return policy, you must have to save this product.' mod='kbmarketplace'}
            </div>
        {else}
        <div class="kb-block kb-form">
            ...
            {if $is_enable_custom_policy}
                <div class="kb-form-field-inblock" style="float:right;">
                        <a href="javascript:void(0)" class="kbbtn btn-info" onclick="addNewPolicy();"><i class="icon-plus"></i><span>{l s='Add New Policy' mod='kbmarketplace'}</span></a>
                </div>
            {/if}
            {* changes over*}
         <ul class="kb-form-list">
            <li class="kb-form-fwidth">
                 ...
                 <div class="kb-form-field-block">
                     <select id="return_manager_policy_select" name="velsof_return_policy" class="kb-inpselect" data-toggle="select2" tabindex="-1" aria-hidden="true">
                        ...
                        {if isset($velsof_return_policy)}
                            {foreach from=$policy item="policy_lang"}
                                {if $policy_lang['return_data_id'] eq $velsof_return_policy}
                                    <option value="{$policy_lang['return_data_id']|escape:'htmlall':'UTF-8'}" selected='selected'>{$policy_lang['value']|escape:'htmlall':'UTF-8'}</option>
                                {else}
                                    <option value="{$policy_lang['return_data_id']|escape:'htmlall':'UTF-8'}">{$policy_lang['value']|escape:'htmlall':'UTF-8'}</option>
                                {/if}
                            {/foreach}
