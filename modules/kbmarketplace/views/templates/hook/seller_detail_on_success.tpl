                        ...
                        {* Display the product line *}
                                <tr>
                                    <td class="cart_product">
                                        <a href="{$link->getProductLink($product['id_product'], $product['link_rewrite'], $product['category'], null, null, $product['id_shop'], $product['id_product_attribute'], false, false, true)|escape:'html':'UTF-8'}"><img src="{$link->getImageLink($product['link_rewrite'], $product['id_image'], 'small_default')|escape:'html':'UTF-8'}" alt="{$product['name']|escape:'html':'UTF-8'}" {if isset($smallSize)}width="{$smallSize.width|escape:'htmlall':'UTF-8'}" height="{$smallSize.height|escape:'htmlall':'UTF-8'}" {/if} /></a>
                                    </td>
                                    ...
                                    {if $product['reference']}<small class="cart_ref">{l s='SKU' mod='kbmarketplace'}{$smarty.capture.default|escape:'htmlall':'UTF-8'}{$product['reference']|escape:'html':'UTF-8'}</small>{/if}
                                ...
                                <td>
                                    <ul>
                                        {if $product['seller_info'] == 'Admin'}
                                            ...
                                            <tr><td class='details_label'>{l s='Address' mod='kbmarketplace'}:</td><td>{$product['seller_info']['address']}</td></tr>{*Variable contains HTML content value,escape not required*}
