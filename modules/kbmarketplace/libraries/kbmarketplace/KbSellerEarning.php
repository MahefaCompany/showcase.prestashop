...
class KbSellerEarning extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'total_earning' => array('type' => self::TYPE_FLOAT, 'validate' => 'isPrice',
                'required' => true),
            'seller_earning' => array('type' => self::TYPE_FLOAT, 'validate' => 'isPrice',
                'required' => true),
            'admin_earning' => array('type' => self::TYPE_FLOAT, 'validate' => 'isPrice',
                'required' => true),
            'is_canceled' => array('type' => self::TYPE_STRING, 'values' => array(
                    '0', '1'), 'default' => '0'),
            'can_handle_order' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt',
                'default' => 0),
    ...
    protected $webserviceParameters = array(
        ...
        'associations' => array(
            'kbsellerorderdetails' => array('getter' => 'getSellerOrderDetailWs',
                'resource' => 'kbsellerorderdetail'),
        )
    );
    ...
    $id_seller, $only_count = false, $start = 0, $limit = 0, $filter_string = null
    )
    {
        ...
        if ($only_count) {
            $sql = str_replace(
                    '{{COLUMN}}', 'IF(COUNT(e.id_seller_earning) IS NULL,0,COUNT(e.id_seller_earning)) as total', $sql
            );
    ...
    public static function getTotalEarningInSellerOrders($id_seller, $include_cancel_order
    = false)
    {
    ...
    public static function getTotalSellerEarning($id_seller, $include_cancel_order
    = false, $custom_filter = '')
    {
    ...
    public static function getTotalSellerSoldProduct($id_seller, $include_cancel_order
    = false, $custom_filter = '')
    {
    ...
    public static function getSellerTotalOrders($id_seller, $not_include_cancel_order
    = false, $custom_filter = '')
    {
        $sql = 'Select IF(COUNT(e.id_seller_earning) IS NULL,0,COUNT(e.id_seller_earning)) as total 
			from '._DB_PREFIX_.'kb_mp_seller_earning as e 
    ...
    public static function getSellerCanceledOrders($id_seller, $custom_filter = '')
    {
        ...
        $sql            = 'Select IF(COUNT(e.id_seller_earning) IS NULL,0,COUNT(e.id_seller_earning)) as total
			...
			INNER JOIN '._DB_PREFIX_.'orders as o on (e.id_order = o.id_order) 
			where e.id_seller = '.(int) $id_seller
                .' AND o.current_state IN ('.implode(',', pSQL($final_statuses)).')';
        ...
        switch ($period) {
            ...
            case 'week':
                ...
                $reportData               = self::getFormattedReportData(
                                $id_seller, $current_week_start_date, $current_week_end_date, $last_week_start_date, $last_week_end_date
                );
                break;
            case 'month':
                ...
                $reportData               = self::getFormattedReportData(
                                $id_seller, $current_month_start_date, $current_month_end_date, $last_month_start_date, $last_month_end_date
                );
                break;
            case 'year':
                ...
                $reportData               = self::getFormattedReportData(
                                $id_seller, $current_year_start_date, $current_year_end_date, $last_year_start_date, $last_year_end_date
                );
                break;
        }
        return $reportData;
    ...
    $id_seller, $first_start_date, $first_end_date, $second_start_date, $second_end_date
    )
    {
        $reportData = array();

        //Get Order Improvement
        $start_date_filter = ' AND (DATE(e.date_add) >= "'.pSQL($first_start_date).'" AND DATE(e.date_add) <= "'.pSQL($first_end_date).'")';
        $current           = self::getSellerTotalOrders($id_seller, false, $start_date_filter);

        $end_date_filter = ' AND (DATE(e.date_add) >= "'.pSQL($second_start_date).'" AND DATE(e.date_add) <= "'.pSQL($second_end_date).'")';
        ...
        $start_date_filter = ' AND (DATE(date_add) >= "'.pSQL($first_start_date).'" AND DATE(date_add) <= "'.pSQL($first_end_date).'")';
        $current           = self::getTotalSellerEarning($id_seller, true, $start_date_filter);

        $end_date_filter = ' AND (DATE(date_add) >= "'.pSQL($second_start_date).'" AND DATE(date_add) <= "'.pSQL($second_end_date).'")';
