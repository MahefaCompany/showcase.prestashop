...
class KbSellerProduct extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'id_shop' => array('type' => self::TYPE_INT, 'validate' => 'isNullOrUnsignedId',
                'copy_post' => false),
            'id_product' => array('type' => self::TYPE_INT, 'validate' => 'isNullOrUnsignedId',
                'copy_post' => false),
    ...
    $id_seller, $id_lang = null, $filters = array(), $only_count = false, $start
    = null, $limit = null, $orderby = null, $orderway = null
    )
    {
        $alias_where = 'p';
                ...
                if ($results && count($results) > 0) {
                    foreach ($results as $p) {
                        ...
                        Db::getInstance(_PS_USE_SQL_SLAVE_)->delete(
                                'kb_mp_seller_product_tracking', pSQL($where)
                        );
                        Db::getInstance(_PS_USE_SQL_SLAVE_)->insert(
                                'kb_mp_seller_product_tracking', array(
    ...
    public static function trackAndUpdateApprovedProduct($id_seller, $product_id
    = 0)
    {
        if ($id_seller > 0) {
            if ($product_id > 0) {
